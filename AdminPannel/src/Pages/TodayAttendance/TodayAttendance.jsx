...
const TodayAttendance = () => {
  ...
  const fetchTodayAttendance = async () => {
    try {
      ...
      const response = await API.get("/teacher-attendance/admin/today");

      console.log("TODAY ATTENDANCE:", response.data);

      const attendanceData = response.data?.data || [];

      const formattedEmployees = attendanceData.map((item) => ({
        id: item._id,
        ...
        break: getBreakTime(item.breaks),

        lastOut: formatTime(item.punchOut),

        totalHours: formatSeconds(item.workSeconds),
  ...
  const executeDeleteRow = () => {
    ...
    setSelectedIds((prev) =>
      prev.filter((id) => id !== deleteTargetEmployee.id),
    );
    ...
    <div className="attendance-container">
      ...
      <div className="attendance-header-section">
        ...
        <div className="attendance-breadcrumbs">
          <HomeIcon size={16} /> <span>&gt;</span> Attendance <span>&gt;</span>{" "}
          <strong>Today</strong>
        </div>
      </div>
      ...
      <div className="attendance-card">
        ...
        <div className="attendance-toolbar">
          ...
          <div className="attendance-action-buttons">
            ...
            {/* <button
              className="attendance-icon-btn attendance-tooltip"
              data-tooltip="add"
                ...
                <label key={colKey} className="column-toggle-item">
                  ...
                  <span>
                    {colKey.charAt(0).toUpperCase() +
                      colKey.slice(1).replace(/([A-Z])/g, " $1")}
                  </span>
                </label>
              ))}
            </div>
        ...
        <div className="attendance-pagination-bar">
          <div className="items-per-page-selector">
            ...
            <div
              className="items-dropdown-trigger"
          ...
          <div className="entry-modal-box" onClick={(e) => e.stopPropagation()}>
            ...
            <form className="entry-modal-body-form" onSubmit={handleFormSubmit}>
                ...
                <div className="form-input-field-group">
                  ...
                  <input
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      setActiveTimeField(
                        activeTimeField === "firstIn" ? null : "firstIn",
                      );
                    }}
                  />
                  ...
                  {activeTimeField === "firstIn" && (
                    <div className="form-time-dropdown-menu">
                      {GENERATED_TIME_OPTIONS.map((t) => (
                        <div
                          key={t}
                          className="form-time-dropdown-option"
                          onClick={() => {
                            setFormFirstIn(t);
                ...
                <div className="form-input-field-group">
                  ...
                  <input
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      setActiveTimeField(
                        activeTimeField === "break" ? null : "break",
                      );
                    }}
                  />
                  <Clock className="form-input-right-icon" size={16} />
                ...
                <div className="form-input-field-group">
                  ...
                  <input
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      setActiveTimeField(
                        activeTimeField === "lastOut" ? null : "lastOut",
                      );
                    }}
                  />
                  <Clock className="form-input-right-icon" size={16} />
              ...
              <div className="form-action-footer-buttons">
                ...
                <button
                  type="button"
                  className="form-cancel-dismiss-btn"
                  onClick={() => setIsEntryModalOpen(false)}
                >
                  Cancel
                </button>
        ...
        <div className="modal-overlay">
          <div className="delete-dialog-box">
            ...
            <div className="delete-dialog-details-list">
              <div>
                <strong>Employee Name:</strong> {deleteTargetEmployee.name}
              </div>
              <div>
                <strong>First In:</strong> {deleteTargetEmployee.firstIn}
              </div>
              <div>
                <strong>Last Out:</strong> {deleteTargetEmployee.lastOut}
              </div>
            </div>
            <div className="delete-dialog-actions">
              <button
                className="delete-confirm-execution-btn"
                onClick={executeDeleteRow}
              >
                Delete
              </button>
              <button
                className="delete-cancel-dismiss-btn"
                onClick={() => setDeleteTargetEmployee(null)}
              >
                Cancel
              </button>
            </div>
